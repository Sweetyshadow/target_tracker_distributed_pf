...
#ifndef TARGET_TRACKER_DISTRIBUTED_PF_DISTRIBUTEDPF3D_H
#define TARGET_TRACKER_DISTRIBUTED_PF_DISTRIBUTEDPF3D_H
...
#include <target_tracker_distributed_pf/ParticleFilterParamsConfig.h>
...
namespace target_tracker_distributed_pf {
    ...
    struct Particle {
        VectorXd state;
        Particle() {};
        Particle(int state_size):state(VectorXd(state_size)) {}
    };
    ...
    private:
        ...
        dynamic_reconfigure::Server<ParticleFilterParamsConfig> dyn_rec_server_;


        //define variables of PF
        std::vector<double> weights;
        std::vector<Particle> particles;

        MatrixXd genTransformMatrix(double velocityDecayTo, double offsetDecayTo, double deltaT);
        ...
        double calculateWeight(VectorXd p, VectorXd m, double std);

    protected:
        static constexpr auto particle_num = 30000;
        static constexpr auto state_size = 6;
    ...
    public:
        ...
        void dynamicReconfigureCallback(ParticleFilterParamsConfig &config, uint32_t level);

        // Initial values for uncertainty on reset
        double  initialUncertaintyPosXY{30},
                initialUncertaintyPosZ{5},
                initialUncertaintyVelXY{5},
                initialUncertaintyVelZ{2},
                initialUncertaintyOffsetXY{1},
                initialUncertaintyOffsetZ{3},
                initialParticlePosXY{0},
                initialParticlePosZ{0.0},
                initialParticleVel{0},
                initialParticleOffset{0};
    ...
    class Callbackhandler {
        public:
        DistributedPF3D *parent;
        Callbackhandler(DistributedPF3D *myparent,bool willbeself, int robot) {
            parent=myparent;
            isSelf=willbeself;
            robotID=robot;
        }
...
#endif //TARGET_TRACKER_DISTRIBUTED_PF_DISTRIBUTEDPF3D_H
